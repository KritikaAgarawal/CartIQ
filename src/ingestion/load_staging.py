...
logger = get_logger('load_staging')
...
def load_csv_to_table(csv_path, table_name, engine):
    ...
    try:
        ...
        if inspector.has_table(table_name):
            ...
            if cols_to_drop:
                for col in cols_to_drop:
                    logger.info(f"Dropping column '{col}' - not present in table {table_name}")
        ...
        logger.error(f"Error: The file {file_path} does not exist. Skipping {table_name}.\n")
...
def main():
    ...
    logger.info("Starting data load into staging tables...\n")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("LOAD SUMMARY - DATABASE ROW COUNTS")
    logger.info("=" * 60)
    ...
    with engine.connect() as conn:
        for _, table_name in load_jobs:
            try:
                ...
                logger.error(f"Table '{table_name}': Error fetching count - {e}")
